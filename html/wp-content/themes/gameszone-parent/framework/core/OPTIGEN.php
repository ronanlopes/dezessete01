...
class TF_OPTIGEN extends TF_TFUSE
{
    ...
    function __construct() {
        ...
        add_action( 'wp_ajax_get_attachments_info', array( $this, 'get_attachments_info' ) );
    }
    ...
    function upload( $opts ) {
        # properties
        {
            ...
            $opts['properties']['class'] = ' upload-input tfuse_option';

            $propstr = $this->propstr( $opts['properties'] );
        }

        wp_enqueue_media();
        add_action( 'admin_footer', array( $this, '_action_upload_enqueue_static') );
        ob_start() ?>
        <input <?php echo $propstr; ?>
            name="<?php echo $opts['id']; ?>" id="<?php echo $opts['id']; ?>"
            type="text"
            value="<?php echo esc_attr( tf_akg( 'value', $opts ) ); ?>"/>
        <div class="upload_button_div">
            <a href="#"
               class="button upload-button"
               id="<?php echo $opts['id']; ?>_button"><?php _e( 'Upload', 'tfuse' ); ?></a>
        </div>
        <?php return ob_get_clean();
    }

    function multi_upload( $opts ) {
        $value = tf_akg('value', $opts);
        ...
        return $this->multi_upload2( $opts );
    }
	...
	function multi_upload2($opts)
	{
		...
		$count = is_array( $value ) ? count( $value ) : count( json_decode( $value, true ) );

        $media = ( ! empty( $opts['media'] ) ) ? 'any' : 'image';
        ...
        foreach ( $value as &$item ) {
            if ( ! isset( $item['id'] ) ) {
                $item['id'] = tfuse_get_attachment_id_by_url( $item['url'] );
                if ( $item['id'] == 0 ) {
                    unset( $item );
                    continue;
                }
            ...
            if ( $media == 'image' ) {
                $item['thumbnail'] = wp_get_attachment_thumb_url( $item['id'] );
            }
        }
        ...
        if ( $media == 'image' ) {
            add_action( 'admin_footer', array( $this, '_action_multi_upload_image_enqueue_static') );
        } else {
            add_action( 'admin_footer', array( $this, '_action_multi_upload_enqueue_static') );
        }

		ob_start(); ?>
		<div class="tfuse-option tfuse-option-multi-upload <?php echo $media; ?>">
			<input type="hidden"
			       class="multi-upload-input"
			       id="<?php echo esc_attr( $opts['id'] ) ?>"
			       name="<?php echo esc_attr( $opts['id'] ) ?>"
			       value="<?php echo esc_attr( json_encode($value) ) ?>"
                   data-type="<?php echo $media; ?>"
			/>
			<div class="thumbs-container">
				<div class="thumb no-image">
					<img src="<?php echo TFUSE_ADMIN_IMAGES . '/no-image.png'; ?>"
					     class="no-image-img" alt="<?php esc_attr_e( 'No image', 'fw' ) ?>"/>
				</div>
                <?php foreach ( $value as $attachment ): ?>
                    <?php if ( $media == 'image' ) : ?>
                        <?php
                        $attachment_thumb_url = $attachment['thumbnail'];
                        $attachment_filename  = basename( get_attached_file( $attachment['id'], true ) );
                        ?>
                        <div class="thumb" data-id="<?php echo esc_attr( $attachment['id'] ); ?>">
                            <img class="item"
                                 src="<?php echo esc_attr( $attachment_thumb_url ); ?>"
                                 alt="<?php echo esc_attr( $attachment_filename ); ?>"/>
                            <a href="#" class="dashicons tfuse-x clear-uploads-thumb"></a>
                        </div>
                        <?php else: ?>
                        <?php
                        $title  = get_the_title($attachment['id']);
                        if ( empty( $title ) ) {
                            $title = basename( get_attached_file( $attachment['id'], true ) );
                        }
                        ?>
                        <div class="thumb" data-id="<?php echo esc_attr( $attachment['id'] ); ?>">
                            <span class="item"
			...
			<p><a href="#" class="upload-button"><?php echo $button_text ?></a></p>
			<div class="templates">
     ...
     * @internal
     **/
    public static function _action_upload_enqueue_static() {
        wp_enqueue_script(
            'tfuse-upload',
            TFUSE_ADMIN_JS . '/upload.js',
            array( 'jquery' )
        );
    }
     ...
     * @internal
     **/
    public static function _action_multi_upload_enqueue_static() {
        wp_enqueue_script(
            'tfuse-multi-upload',
            TFUSE_ADMIN_JS . '/multi-upload.js',
     ...
     * @internal
     **/
    public static function _action_multi_upload_image_enqueue_static() {
        wp_enqueue_script(
            'tfuse-multi-upload-image',
            TFUSE_ADMIN_JS . '/multi-upload-image.js',
            array( 'jquery', 'jquery-ui-sortable' )
        );
    }
    ...
    public function get_attachments_info() {
         ...
         */
        global $TFUSE;
        $items  = (array)$TFUSE->request->POST( 'items' );
        $response = array();
        ...
        foreach ( $items as $item ) {
            $id = tf_akg('id', $item);
            $url = tf_akg('url', $item);
            $fields = (array)tf_akg('fields', $item);
            ...
            if ( ! empty( $id ) ) {
                ...
                $post = get_post( tfuse_get_attachment_id_by_url( $url ) );
            }
            ...
            foreach ( $fields as $field ) {
                array( 'id', 'url', 'thumbnail', 'title', 'description' );
                ...
                switch ( $field ) {
                    case 'id' :
                        ...
                        break;
                    case 'url' :
                        $temp['url'] = wp_get_attachment_url( $post->ID );
                        break;
                    case 'thumbnail' :
                        $temp['thumbnail'] = wp_get_attachment_thumb_url( $post->ID );
                        break;
                    case 'title' :
