...
class TF_INTERFACE extends TF_TFUSE
{
    ...
    public function admin_add_js()
    {
        ...
        wp_register_script('maps.google.com', '//maps.google.com/maps/api/js?sensor=false' . tfuse_gmap_key(), array('jquery'), '1.0', true);
        wp_register_script(
            'tfuse-multi-upload',
        ...
        if ($allow_enqueue) {
            ...
            @wp_enqueue_script('tfuse-multi-upload');
            @wp_enqueue_script('tfuse-multi-upload-image');
        }
    }
    ...
    public function taxonomy_row_template($row = array())
    {
        ...
        if (method_exists($this->optigen, $row['type'])) {
            ...
            if ('upload' == $row['type']) {
                ...
                $out .= '<table class="tfuse-tax-upload-form option-upload"><tr><td>';
                $out .= $this->optigen->$method($row);
