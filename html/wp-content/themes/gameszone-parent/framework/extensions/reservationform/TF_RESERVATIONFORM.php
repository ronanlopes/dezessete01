...
class TF_RESERVATIONFORM extends TF_TFUSE
{
    ...
    function list_reservations()
    {
        ...
        $post_statuses = array( 1 => 'any', 2 => 'private', 3 => 'publish', 4 => 'draft', 5 => 'trash' );
        $this->common_html();
        ...
        if ($this->request->isset_GET( 's' )) {
            ...
            $only_for_counting = new WP_Query(
                array_merge( $args, array( 'posts_per_page' => -1, 'post_status' => array('any') ) )
            );
            $tmp_posts         = $only_for_counting->get_posts();
            wp_reset_postdata();
            $query = new WP_Query( $args );
            $posts = $query->posts;
            wp_reset_postdata();
        }
        $trash_posts = new WP_Query( array(
            'posts_per_page' => - 1,
            'fields'         => 'ids',
            'post_type'      => 'reservations',
            'post_status'    => 'trash'
    ...
    function submitFrontendForm()
    {
        ...
        if ($status) {
            ...
            $form['email_subject'] = apply_filters(
                'tfuse_reservationform_admin_email_subject',
                $form['email_subject']
            );
