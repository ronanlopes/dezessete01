...
<div id="posts-filter">
    <?php
    $array_s          = array( 1 => 'all', 2 => 'pending', 3 => 'approved', 4 => 'rejected', 5 => 'trash' );
    ...
    <a class="<?php echo ( $is_filter && $is_filter == 1 ) ? 'current_filter_type' : ''; ?>"
       href="admin.php?page=tf_reservations_list&filter=1"><?php _e( 'All', 'tfuse' ) ?> <span
            class="count">(<?php echo $statistic['all'] ?>)</span></a> |
    <a class="<?php echo ( $is_filter && $is_filter == 2 ) ? 'current_filter_type' : ''; ?>"
       href="admin.php?page=tf_reservations_list&filter=2"><?php _e( 'Pending', 'tfuse' ) ?> <span
            class="count">(<?php echo $statistic['pending'] ?>)</span></a> |
    <a class="<?php echo ( $is_filter && $is_filter == 3 ) ? 'current_filter_type' : ''; ?>"
       href="admin.php?page=tf_reservations_list&filter=3"><?php _e( 'Approved', 'tfuse' ) ?> <span
            class="count">(<?php echo $statistic['approved'] ?>)</span></a> |
    <a class="<?php echo ( $is_filter && $is_filter == 4 ) ? 'current_filter_type' : ''; ?>"
       href="admin.php?page=tf_reservations_list&filter=4"><?php _e( 'Rejected', 'tfuse' ) ?> <span
            class="count">(<?php echo $statistic['rejected'] ?>)</span></a>
    <a class="<?php echo ( $is_filter && $is_filter == 5 ) ? 'current_filter_type' : ''; ?>"
       href="admin.php?page=tf_reservations_list&filter=5"><?php _e( 'Trash', 'tfuse' ) ?> <span
            class="count">(<?php echo $statistic['trash'] ?>)</span></a>
    ...
    <form id="reservations_search" action='admin.php?page=tf_reservations_list' method='get'>
        <p class="search-box">
            ...
            <input id="ssubmit" class="button" type="submit" value="<?php _e( "Search Res. Number", 'tfuse' ); ?>"
                   name="">
        </p>
    </form>
</div>
<div class="filter_second tablenav top">
    <?php if( $TFUSE->request->GET( 'filter' ) == 5 ) : ?>
        <a class="delete_selected_reservations button"><?php _e( 'Delete Selected', 'tfuse' ) ?></a>
    <?php else :?>
        <a class="trash_selected_reservations button"><?php _e( 'Trash', 'tfuse' ) ?></a>
    <?php endif ?>
    ...
    <div class="tablenav-pages">
		<span class="displaying-num"><?php echo $statistic[ $array_s[ $filter_type ] ] ?><?php _e( 'items',
                'tfuse' ) ?> </span>
<span class="pagination-links <?php echo $pagination_class; ?>"><a href="<?php echo $url; ?>1"
                                                                   title="<?php _e( "Go to the first page",
                                                                       'tfuse' ); ?>" class="first-page">«</a>
<a href="<?php echo $url . ( $paged - 1 ); ?>" title="<?php _e( "Go to the previous page", 'tfuse' ); ?>"
   class="prev-page">‹</a>
<span class="paging-input"><input type="text" size="1" value="<?php echo $paged ?>" name="paged"
                                  title="<?php _e( "Current page", 'tfuse' ); ?>" class="current-page"> of <span
...
<table cellspacing="0" class="wp-list-table widefat fixed pages form_list_table">
    ...
    $post_statuses = array(
        ...
        'draft'   => __( 'Rejected', 'tfuse' ),
        'private' => __( 'Pending', 'tfuse' ),
        'trash'   => __( 'Trash', 'tfuse' ),
    );
    if ( isset( $posts[0]->ID ) > 0 ) {
        ...
        foreach ( $posts as $post ) {
            ...
            <tr valign="top"
                ...
                <span class="edit">
                    ...
                    ><?php _e( 'Edit', 'tfuse' ) ?></a> |
                        ...
                        <?php if ( $post->post_status == 'trash' ) : ?>
                            <span class="trash">
                <a href="#" title="<?php _e( "Delete this item", 'tfuse' ); ?>"
                   ...
                   class="tf_delete_reservation">
                    <?php _e( 'Delete', 'tfuse' ) ?>
                        ...
                        <?php else : ?>
                            <span class="trash">
                <a href="#" title="<?php _e( "Move to trash", 'tfuse' ); ?>"
                   rel="<?php echo $post->ID ?>"
                   class="tf_trash_reservation">
                    <?php _e( 'Trash', 'tfuse' ) ?>
                </a>
                </span>
                        <?php endif ?>
                ...
                <td>
                    <?php echo ( $date_out != '' ) ? date_i18n( get_option( 'date_format' ),
                        strtotime( $date_out ) ) : "-";
                    $date_out = ''; ?>
                </td>
                <td>
