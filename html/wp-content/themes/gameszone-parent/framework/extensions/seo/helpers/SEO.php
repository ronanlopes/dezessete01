...
class TF_SEO_SITEMAP {
	...
	function RemoteOpen( $url, $method = 'get', $postData = null, $timeout = 10 ) {
		...
		if ( floatval( $wp_version ) < 2.7 ) {
			...
			if ( $s->status != "200" ) {
				trigger_error( 'Snoopy Web Request failed: Status: ' . $s->status . "; Content: " . htmlspecialchars( $s->results ),
					E_USER_NOTICE );
			}
	...
	function BuildSitemap() {
		...
		if ( true ) {
			if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) {
				...
				if ( $p ) {
					$homePid = $p->ID;
					$this->AddUrl( trailingslashit( $home ),
						$this->GetTimestampFromMySql( ( $p->post_modified_gmt && $p->post_modified_gmt != '0000-00-00 00:00:00' ? $p->post_modified_gmt : $p->post_date_gmt ) ),
						$this->GetOption( "cf_home" ),
						$this->GetOption( "pr_home" ) );
				}
			} else {
				$this->AddUrl( trailingslashit( $home ),
					$this->GetTimestampFromMySql( get_lastpostmodified( 'GMT' ) ),
					$this->GetOption( "cf_home" ),
					$this->GetOption( "pr_home" ) );
			}
		}
		...
		if ( true ) {
			...
			if ( true ) {
				...
				foreach ( get_post_types() as $customType ) {
					if ( ! in_array( $customType, array( 'revision', 'nav_menu_item', 'attachment' ) ) ) {
						if ( ! tfuse_options( 'seo_xmls_exclude_posttype_' . $customType,
								false ) && 'STOP!' != tfuse_options( 'seo_xmls_exclude_posttype_' . $customType,
								'STOP!' )
						) {
							$where .= ",'$customType'";
						}
			...
			$postCount = intval( $wpdb->get_var( "SELECT COUNT(*) AS cnt FROM `" . $wpdb->posts . "` WHERE " . $where,
				0,
				0 ) );
			...
			if ( $postRes ) {
				...
				foreach ( $postRes as $post ) {
					...
					if ( $permalink != $home && $post->ID != $homePid ) {
						...
						//Add it
						$this->AddUrl( $permalink,
							$this->GetTimestampFromMySql( ( $post->post_modified_gmt && $post->post_modified_gmt != '0000-00-00 00:00:00' ? $post->post_modified_gmt : $post->post_date_gmt ) ),
							( $isPage ? $cf_pages : $cf_posts ),
							$prio );
						...
						if ( false ) {
							...
							for ( $p = 1; $p <= $post->postPages; $p ++ ) {
								if ( get_option( 'permalink_structure' ) == '' ) {
									...
									$subPage = trailingslashit( $permalink ) . user_trailingslashit( $p + 1,
											'single_paged' );
								}
			...
			foreach ( get_taxonomies() as $taxName ) {
				if ( ! in_array( $taxName, array( 'nav_menu', 'link_category', 'post_format' ) ) ) {
					...
					if ( isset( $taxonomy->labels->name ) && trim( $taxonomy->labels->name ) != '' ) {
						if ( ! tfuse_options( 'seo_xmls_exclude_taxonomy_' . $taxName,
								false ) && 'STOP!' != tfuse_options( 'seo_xmls_exclude_taxonomy_' . $taxName, 'STOP!' )
						) {
							if ( $taxonomy ) {
			...
			if ( count( $taxList ) > 0 ) {
				...
				foreach ( $termInfo AS $term ) {
					if ( ! in_array( $term->_taxonomy, array( 'nav_menu', 'link_category', 'post_format' ) ) ) {
						$this->AddUrl( get_term_link( $term->slug, $term->_taxonomy ),
							$term->_mod_date,
							$this->GetOption( "cf_tags" ),
							$this->GetOption( "pr_tags" ) );
					}
				}
			}
		}
		...
		if ( ! empty( $pingUrl ) ) {
			...
			if ( $pingres == null || $pingres === false || strpos( $pingres,
					"successfully received and added" ) === false
