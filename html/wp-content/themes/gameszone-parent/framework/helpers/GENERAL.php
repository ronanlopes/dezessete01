 ...
 */
{
    ...
    function tfuse_get_tweets($username, $count=20){
        ...
        $basedir = get_template_directory().'/cache';
        $tweets_cache_path = $basedir . '/twitter_json_'.$username.'_rpp_'.$count.'.cache';
    ...
    function tfuse_get_gallery_images($post_id,$input_id) {
        ...
        $option = str_replace( TF_THEME_PREFIX . '_', '', $input_id );
        $images = tfuse_page_options( $option, null, $post_id );
        $id_key = 'id';
        ...
        return get_posts( array(
            ...
            'include'   => wp_list_pluck( $images, $id_key ),
            'orderby'   => 'post__in'
        ) );
    }
...
function tfuse_gmap_key( $query = true ) {
    $key = apply_filters( 'tfuse_gmap_key', tfuse_options( 'gmap_key', '' ) );
    if ( ! $query || empty( $key ) ) {
        return $key;
    }
