...
if (!function_exists('tfuse_get_game_posts')) :
...
if( !function_exists('tfuse_add_meta_query_ratings') ):
    function tfuse_add_meta_query_ratings(){
        $ratings_exists = get_option( 'tfuse_ratings' );

        if($ratings_exists == true) return;
    ...
    add_action('admin_head','tfuse_add_meta_query_ratings',1);
    add_action('wp_head','tfuse_add_meta_query_ratings',1);
endif;

if (!function_exists('tfuse_save_review_rating')) :
    function tfuse_save_review_rating($post_id)
    {
        global $TFUSE;
        ...
        $rating = $TFUSE->request->post(TF_THEME_PREFIX.'_rating');
        update_post_meta($post_id, 'tfuse_rating', $rating);
    }
endif;
add_action( 'save_post_review', 'tfuse_save_review_rating' );
